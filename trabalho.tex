...
Além da utilização dos \textit{endpoints} e métodos HTTP, é necessário um padrão de dados para que ambos, usuário e API, tenham uma estrutura pré definida de possibilidades de envio e retorno. Dessa forma, o padrão de dados \textbf{JSON}, proposto por \citeonline{crockford2006application} foi o escolhido para servir como representação de dados da API, desde o recebimento de novos recursos (itens, usuários, etc.) até o retorno das recomendações. Um exemplo da representação JSON para as recomendações pode ser visto na figura \ref{json_example}.
...
Em resumo a combinação entre a modificação de recursos via \textit{endpoints} e a estrutura JSON, como visto no quadro \ref{resource_example} e na figura \ref{json_example}, tornam-se uma ótima escolha, uma vez que seja pela simplicidade com que as estruturas geradas pela API são representadas ou pela legibilidade que os \textit{endpoints} trazem ao fluxo de envio de retorno de dados.
