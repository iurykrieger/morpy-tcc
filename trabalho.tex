...
\section{COMPUTAÇÃO DE ALTO DESEMPENHO EM GPU}

Com a rápida evolução e ubiquidade das GPUs, apresentada por \citeonline{nickolls2010gpu}, as unidades de processamento gráfico (\textbf{GPU}) antes utilizadas apenas como processadores gráficos, hoje são utilizadas como processadores paralelos e tornaram-se a plataforma com o maior custo benefício, executando milhares de threads ao mesmo tempo. Além disso, a GPU permite aplicações antes inviáveis, devido a programas com longo tempo de execução. 

Além disso, \citeonline{fan2004gpu} apresentam a GPU como uma plataforma projetada para ser extremamente rápida no processamento de grandes conjuntos de dados. Porém, para fazer proveito do design e rapidez da GP, tornando seu uso viável, é necessário que as aplicações que manuseiam estes dados compartilhem de algumas características \cite{owens2008gpu}:

\begin{itemize}

	\item \textbf{Requerimento computacional grande:} aplicações que necessitam de resposta em tempo real (ou perto disso) se beneficiam muito da arquitetura de uma GPU, conseguindo entregar milhares de operações paralelas por segundo.

 	\item \textbf{Paralelismo substancial:} devido a arquitetura da GPU possuir muito mais núcleos e unidades de paralelismo que um processador comum, executar tarefas originalmente independentes, tais como operações em vértices e matrizes, torna-se muito mais rápido.

	\item \textbf{Vazão mais importante que latência:} aplicações em que a vazão de dados é mais importante que a latência, ou seja, onde o tempo de processamento é o gargalo computacional, não o tempo de acesso. 

\end{itemize}

Analisando as características apresentadas por \citeonline{owens2008gpu}, é possível perceber a semelhança entre as aplicações que usufruem da GPU e os sistemas de recomendação. Levando em consideração que a matriz $n \times m$ de avaliações dos usuários pode chegar a milhares ou até milhões de avaliações a serem processadas \cite{gomez2016netflix}, a utilização da GPU para o processamento destes dados torna-se viável.

Assim como a aplicação apresentada por \citeonline{do2013filtragem}, este trabalho pretende aproveitar os ganhos da utilização de GPUs no processamento da matriz de similaridades. Estes ganhos podem chegar em uma redução de tempo de até cem vezes, através da utilização da tecnologia \textbf{CUDA} \cite{luebke2008cuda}, podendo fazer a diferença na utilização de um sistema de recomendação como serviço.
