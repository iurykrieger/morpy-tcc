...
Do mesmo modo que a biblioteca desenvolvida por \citeonline{brozovsky2007recommender}, a API proposta neste trabalho também visa ser multipropósito e distribuída como código aberto pela licença pública GNU (\textbf{GPL}), porém, fornecendo tais funcionalidades como um serviço web independente de linguagem de programação, o que não acontece nos exemplos apresentados.
...
Fazer uma tabela com todos os endpoints.
...
Como parte central do diferencial de funcionamento da aplicação, os metadados são eficientes na construção de estruturas de dados construídas dinamicamente, como abordado na seção \ref{}, incertas até sua execução. Sendo assim, os mesmos formam a base que sustenta os dados desta aplicação.
...
\begin{figure}[h!tp]
	...
	\caption{\label{validation}Exemplo de validação baseada nos metadados.}
...
Além da validação dos novos dados, a construção da estrutura de retorno também é feita com base nos metadados em vigor. O atributo declarado como identificar único (\textbf{\textit{key:true}}) nos metadados é obrigatoriamente adicionado na estrutura de retorno, uma vez que é utilizado como parâmetro base em todos os \textit{endpoints} relacionados. Além disso, o atributo \textbf{\textit{hide}} define se o atributo existente será exibido ou não no JSON de retorno.

Para ilustrar o processo, a figura \ref{} demonstra um exemplo do fluxo interno do \textbf{analisador de dados}, desde o recebimento da informação, até a persistência e posterior retorno da mesma. O código completo dos modelos de dados pode ser visto no apêndice \ref{}. Exemplificar a construção do retorno contrastando a inserção, os metadados e o retorno JSON final. \ref{} FIGURA!
...
Criar outra figura pra explicar as 3 partes: requisição do usuário via HTTP com os dados pra atualizar, persistência e retorno | em paralelo: geração e persistência das recomendações e retorno das recomendações em outra requisição futura.
...
Explicar as chaves dinâmicas com base no atributo key do usuário e porque não usar a chave _id. Pra ficar mais transparente pro usuário e tals...
